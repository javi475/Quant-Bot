...
export function LoginPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      const result = await login.mutateAsync({ username, password, totp_code: totpCode });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950">
      ...
      >
        ...
        <p className="mb-6 text-sm text-slate-400">Sign in with your operator credentials and 2FA code.</p>
        ...
        <label className="mb-1 block text-xs font-medium text-slate-400" htmlFor="totp">
          2FA code
        </label>
        <input
          id="totp"
          ...
          required
          className="mb-6 w-full rounded border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100 focus:border-blue-500 focus:outline-none"
        />
